<script lang="ts">
    ...
    let coverPreview: string | null = $state(post.cover_image ?? null);
    let isDraggingOver = $state(false);
    let existingCover = $state(post.cover_image ?? null);
    ...
    function handleCoverFile(file: File) {
        ...
        if (coverPreview && coverPreview !== existingCover) {
    ...
    function toggleCodeBlock() { editor?.chain().focus().toggleCode().run(); }
...
<svelte:head>
    <title>Chỉnh sửa bài viết - Dashboard</title>
</svelte:head>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between mb-stack-lg gap-4">
                <div>
                    <nav class="flex items-center gap-2 text-on-surface-variant text-sm mb-1">
                        ...
                        <span class="text-primary font-medium">Chỉnh sửa bài viết</span>
                    </nav>
                    <h1 class="font-headline-lg text-headline-lg text-primary">Chỉnh Sửa Bài Viết</h1>
                </div>
                <div class="flex items-center gap-stack-md">
                    <button
                        onclick={() => submitPost('draft')}
                        disabled={form.processing}
                        class="px-6 py-3 rounded-full border-2 border-primary text-primary font-label-bold hover:bg-primary/5 transition-all active:scale-95"
                    >
                        Lưu bản nháp
                    </button>
                    <button
                        ...
                        disabled={form.processing}
                        class="px-8 py-3 rounded-full bg-primary text-on-primary font-label-bold duolingo-button border-b-4 border-primary-container hover:brightness-110 transition-all active:scale-95 disabled:opacity-50"
                    >
                        Đăng bài
                    </button>
                </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
                ...
                <div class="lg:col-span-8 flex flex-col gap-gutter">
                    ...
                    {#if coverPreview}
                        ...
                        <div class="glass-card rounded-xl overflow-hidden relative group">
                            ...
                            {#if existingCover}
                                <div class="absolute top-2 left-2 bg-black/50 text-white text-xs px-2 py-1 rounded-lg">
                                    Ảnh hiện tại
                                </div>
                            {/if}
                    ...
                    {:else}
                        ...
                        <div
                            class="glass-card rounded-xl p-stack-md flex flex-col items-center justify-center min-h-[200px] border-2 border-dashed transition-all cursor-pointer group {isDraggingOver ? 'border-primary bg-primary/5 scale-[1.01]' : 'border-outline-variant hover:border-primary/50'}"
                        ...
                        >
                            <div class="w-14 h-14 rounded-full bg-primary/10 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                                <span class="material-symbols-outlined text-primary text-3xl">{isDraggingOver ? 'file_download' : 'image'}</span>
                            </div>
                            <h3 class="font-title-md text-primary">{isDraggingOver ? 'Thả ảnh tại đây' : 'Thay ảnh bìa'}</h3>
                            <p class="text-on-surface-variant mt-1 text-center text-sm">Kéo thả hoặc nhấp để chọn ảnh mới<br/><span class="text-xs opacity-70">(Tối đa 5MB)</span></p>
                    ...
                    <div class="glass-card rounded-xl flex flex-col min-h-[600px] overflow-hidden border border-outline-variant/30">
                        ...
                        {#if editor}
                            <div class="flex flex-wrap items-center gap-1 p-3 bg-surface-container border-b border-outline-variant/20 shadow-sm" data-trigger={editorUpdateTrigger}>
                                ...
                                <button onclick={toggleCodeBlock} class="p-2 hover:bg-surface-variant rounded-lg transition-all {editor.isActive('code') ? 'bg-surface-variant' : ''}" title="Code"><span class="material-symbols-outlined">code</span></button>
                ...
                <div class="lg:col-span-4 flex flex-col gap-gutter">
                    ...
                    <div class="glass-card rounded-xl p-4 flex items-center gap-3">
                        <div class="w-2 h-2 rounded-full {post.status === 'published' ? 'bg-green-500' : 'bg-amber-400'}"></div>
                        <span class="text-sm font-label-bold text-on-surface-variant">
                            {post.status === 'published' ? 'Đang hiển thị' : 'Bản nháp'}
                        </span>
                        <span class="ml-auto text-xs text-outline-variant">ID: #{post.id}</span>
                    </div>
                    ...
                    <!-- Danger Zone -->
                    <div class="glass-card rounded-xl p-stack-lg border border-error-container/30">
                        <h3 class="font-title-md text-error flex items-center gap-2 mb-3">
                            <span class="material-symbols-outlined">warning</span> Vùng nguy hiểm
                        </h3>
                        <button
                            type="button"
                            onclick={() => {
                                if (confirm('Bạn có chắc muốn xóa bài viết này? Hành động không thể hoàn tác.')) {
                                    import('@inertiajs/svelte').then(({ router }) => {
                                        router.delete(`/dashboard/posts/${post.id}`);
                                    });
                                }
                            }}
                            class="w-full px-4 py-2.5 rounded-xl border border-error text-error font-label-bold hover:bg-error-container/30 transition-colors flex items-center justify-center gap-2"
                        >
